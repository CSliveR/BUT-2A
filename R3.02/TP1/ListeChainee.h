...
bool ListeChainee<TypeInfo>::insereAtPositIter(const int nouvellePosition, const TypeInfo &nouvelleInfo) {
    ...
    if(nouvellePosition == 1) {
        celluleAInserer->setSuivante(ptrTete);
        ptrTete = celluleAInserer;
    }else {
        for(unsigned int i=2; i < nouvellePosition - 1; i++) {
            celluleCourante = celluleCourante->getSuivante();
        }
        /*Attention à l'ordre des instructions, on travaille d'abord sur la celluleAInserer seule puis on l'insère dans la liste*/
        celluleAInserer->setSuivante(celluleCourante->getSuivante());
        celluleCourante->setSuivante(celluleAInserer);
    }
...
bool ListeChainee<TypeInfo>::supprimeAtPositIter(int position) {
     ...
     * À COMPLETER
     */
    if(position < 1 || position > nbCellules + 1) {
        return false;
    }
    ...
    if(position == 1) {
        ...
        delete celluleCourante;
    }else {
        for(int i=2; i < position - 1; i++) {
            celluleCourante = celluleCourante->getSuivante();
        }
        Cellule<TypeInfo> *ptrCellSuppr = celluleCourante->getSuivante();
        celluleCourante->setSuivante(ptrCellSuppr->getSuivante());
        delete ptrCellSuppr;
    }
    nbCellules = nbCellules - 1;
    return true;
...
bool ListeChainee<TypeInfo>::estInfoPresentRecWorker(Cellule<TypeInfo> *ptrCetteListe, TypeInfo &infoCible) const {
    ...
    if(nbCellules == 1) {
        return ptrCetteListe->getInfo() == infoCible;
    }else {
        nbCellules = nbCellules - 1;
        return estInfoPresentRecWorker(ptrCetteListe->getSuivante(), infoCible);
    }*/
    if(ptrCetteListe->getSuivante() == nullptr) {
        return false;
...
void ListeChainee<TypeInfo>::supprimeAtPositRecWorker(Cellule<TypeInfo>*& ptrCetteListe, int position) {
     ...
     */
    if (position == 1){
        Cellule<TypeInfo> *ptrCellSuppr = ptrCetteListe;
        ptrCetteListe = ptrCetteListe->getRefSuivante();
        supprimeTeteWorker(ptrCellSuppr);
        delete ptrCellSuppr;
        nbCellules--;
    }else {
